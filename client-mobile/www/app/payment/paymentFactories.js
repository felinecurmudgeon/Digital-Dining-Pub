...
.factory('CheckFactory', ['$http', '$window', function ($http, $window) {
  ...
  var chargeCard = function (paymentSummary) {
    return $http({
      url: window.isMobileDev ? 'http://localhost:8000/api/charges' : 'http://52.33.58.174:8000/api/charges',
      method: 'POST',
      data: paymentSummary
    }).then(function () {
  ...
  var closeBill = function () {
    return $http({
      ...
      url: window.isMobileDev ? 'http://localhost:8000/api/parties/' + $window.localStorage.getItem('partyId') + '?event=close' : 'http://52.33.58.174:8000/api/parties/' + $window.localStorage.getItem('partyId') + '?event=close',
      method: 'PUT'
    })
    .then(function () {
...
.factory('PaymentFactory', ['$http', function ($http) {
  var addCard = function (token) {
    return $http({
      url: window.isMobileDev ? 'http://localhost:8000/api/charges/addcard' : 'http://52.33.58.174:8000/api/charges/addcard',
      method: 'POST',
      data: {
        'stripeToken' : token
      }
