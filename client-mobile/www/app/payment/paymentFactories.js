...
.factory('CheckFactory', ['$http', '$window', function ($http, $window) {
  ...
  var chargeCard = function (paymentSummary) {
    return $http({
      url: window.isDevProduction ? 'http://localhost:8000/api/charges' : 'http://ec2-52-33-106-186.us-west-2.compute.amazonaws.com/api/charges',
      method: 'POST',
      data: paymentSummary
    }).then(function () {
  ...
  var closeBill = function () {
    return $http({
      ...
      url: window.isDevProduction ? 'http://localhost:8000/api/parties/' + $window.localStorage.getItem('partyId') + '?event=close' : 'http://ec2-52-33-106-186.us-west-2.compute.amazonaws.com/api/parties/' + $window.localStorage.getItem('partyId') + '?event=close',
      method: 'PUT'
    })
    .then(function () {
...
.factory('PaymentFactory', ['$http', function ($http) {
  var addCard = function (token) {
    return $http({
      url: window.isDevProduction ? 'http://localhost:8000/api/charges/addcard' : 'http://ec2-52-33-106-186.us-west-2.compute.amazonaws.com/api/charges/addcard',
      method: 'POST',
      data: {
        'stripeToken' : token
      }
